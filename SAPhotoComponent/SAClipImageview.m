...
@interface SAClipImageview() <UIGestureRecognizerDelegate>
...
@end
@implementation SAClipImageview
...
- (id)initWithCropImage:(UIImage*)cropImage cropSize:(CGSize)cropSize {
    ...
    if(self)
    {
        ...
        _imageScale = (self.inputImage.size.width/self.inputImage.size.height) * 0.59;
        
       
        [self initImageView:cropImage];
        
        _imgViewframeInitValue = _imgView.frame;
        _imgViewcenterInitValue = _imgView.center;
        _realCropsize = cropSize;
        
        _cropRect = CGRectMake(0, 0, cropSize.width,cropSize.height);
...
- (void)initImageView:(UIImage *)cropImage{
    
     CGRect imgViewBound = CGRectMake(0, 0,cropImage.size.width * _imageScale,cropImage.size.height * _imageScale);
